...
mod = SourceModule("""
    ...
    __global__ void productoPunto( float *a, float *b, float *c ) {
        ...
        int aux = threadIdx.x + blockIdx.x * blockDim.x;
        temp[threadIdx.x] = a[aux] * b[aux];
        __syncthreads();
...
productoPunto = mod.get_function("productoPunto")

productoPunto(cuda.In(a), cuda.In(b), cuda.Out(c), 
